...
export class CourseRepository implements ICourseRepository {
  ...
  //   async  getCoursesByCategory (category:string, page:number,itemsPerPage:number) :Promise<{
  //     courses: ICourse[];
  //     totalCount: number;
  // } | null>  {
  //     const filter = category ? { category, approval: "Approved", status: true } : { approval: "Approved", status: true };
  //     const skip = (page - 1) * itemsPerPage;
  //     const totalCount = await Course.countDocuments(filter);
    
  //     const courses = await Course.find(filter)
  //     .populate('category') // Populate the category field with its actual data
  //     .skip(skip)
  //     .limit(itemsPerPage);
  //     console.log(courses, "courses in getcoursesb category")
  //     return { courses, totalCount };
  //   };
    
  async getCoursesByCategory(categoryId: string, page: number = 1, limit: number = 10): Promise<{ courses: ICourse[], total: number }> {
    // Filtering criteria for approved and active courses in a given category
    const filter = { category: categoryId, approval: "Approved", status: true };

    // Pagination calculation
    const skip = (page - 1) * limit;

    // Counting total courses for the given category
    ...
    const courses = await Course.find(filter)
        ...
        .skip(skip)
        .limit(limit)
        .sort({ createdAt: -1 }); // Optionally sort courses by creation date (newest first)
